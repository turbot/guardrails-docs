...
**Alerts**: Automated messages from Guardrails when a Policy violation or significant event occurs. These alerts appear in the Guardrails console. They can also be configured to notify users via email, Slack, Microsoft Teams, or HTTP endpoints.
...
**Resource**: Represents cloud platform objects that are tracked by the CMDB. A Resource is a specific instance of a Resource Type. May have many Policy Values and Controls associated with it.
