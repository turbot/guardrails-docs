...
These objectives range from P1 (blocking public S3 buckets, requiring database encryption) to P3 (soft delete for recovery, advanced key management). The priority depends on the data sensitivity and compliance requirements. Public healthcare data requires stronger preventions than internal development data.
...
Most Identity & Access objectives are P1 or P2 because compromised credentials are a primary attack vector. Requiring MFA for root accounts, eliminating root access keys, enforcing MFA for all users, and preventing wildcard administrative policies are foundational preventions.
...
Public access prevention is typically P1—accidentally public S3 buckets and databases are common breach sources. Cross-account access preventions are typically P2 or P3 depending on your multi-account strategy. If you heavily use cross-account access, these preventions become more important.
