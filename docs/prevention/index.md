...
![PSPM Architecture Diagram](./turbot-pspm-architecture.png)


Turbot Guardrails enables [prevention-first cloud security](https://turbot.com/prevention/library), stopping misconfigurations, policy violations, and risky deployments before they reach production.

## Understanding Prevention-First Security

Most security tools tell you what's wrong after something's already deployed, like a public S3 bucket, an unencrypted database, or a security group open to the world. Prevention-first security stops these issues before they reach production. Guardrails discovers the preventions already in your environment (Service Control Policies, Azure Policies, account settings, etc.), maps them to security objectives, and scores how well you're preventing issues before they can occur.

## Key Concepts

### Objectives

[Objectives](/guardrails/docs/prevention/objectives) are the security goals and outcomes you're trying to achieve. "Restrict AWS resources to allowed regions" is an objective. "Require encryption at rest for AWS EBS volumes" is another. "Prohibit public access to S3 buckets" is a third. These aren't preventive mechanisms; they're the security requirements you need to meet.
...
[Benchmarks](/guardrails/docs/prevention/objectives/benchmarks) are compliance frameworks that organize objectives, like AWS CIS v6.0.0, NIST 800-53 Rev 5, and Azure CIS v5.0.0. Each benchmark shows your prevention score for that framework, helping you track progress toward compliance certification. Many objectives appear in multiple benchmarks, so implementing one prevention can improve multiple framework scores simultaneously.
...
**Perfect scores aren't always the goal.** Some organizations set practical targets like "All P1 objectives must score 4+ in production accounts. P2 objectives should score 3+. P3-P5 are discretionary." This creates clear expectations and measurable goals without demanding perfection everywhere.
