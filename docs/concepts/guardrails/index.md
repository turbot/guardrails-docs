...
| [Allowed](concepts/guardrails/allowed)                             | Verify whether specific resource attributes meet requirements with independent, granular controls for each objective (region, instance type, encryption, etc.).     |
| [Approved](concepts/guardrails/approved)                           | [DEPRECATED] Verify whether a particular resource is allowed to exist and take an appropriate action if not. Use [Allowed](concepts/guardrails/allowed) instead.    |
